...
/**
 * Turn retrieved chunks into a system-prompt addendum. Numbered so the model
 * can point back at "[2]" internally while still citing by act+section in
 * its bolded citations, per SYSTEM_PROMPT's existing format rules.
 */
function buildRagContextBlock(context: ScoredChunk[]): string {
  if (context.length === 0) return '';

  const passages = context
    .map((c, i) => `[${i + 1}] ${c.lawTitle} — ${c.section}\n${c.text}`)
    .join('\n\n---\n\n');

  return `

## Retrieved Legal Context
The passages below were retrieved from this app's verified law repository as the most relevant to the user's question. Ground your answer in them wherever they apply, and cite the exact act + section shown (not the bracketed number, which is only for your reference).

${passages}

If none of the passages actually answer the question, say so explicitly and fall back to your general knowledge of Indian law, clearly noting that it isn't from the verified repository.`;
}
...
export async function sendChatMessage(
  ...
  const messages: GroqMessage[] = [
    { role: 'system', content: SYSTEM_PROMPT + buildRagContextBlock(context) },
  ];
